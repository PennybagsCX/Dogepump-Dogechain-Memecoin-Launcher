...
async function main() {
  console.log("Deploying DogePump DEX to Dogechain Mainnet...");
  ...
  console.log(`npx hardhat verify --network dogechain ${factory.address} ${deployer.address}`);
  console.log(`npx hardhat verify --network dogechain ${router.address} ${factory.address} ${WDOGE_TOKEN}`);
  console.log(`npx hardhat verify --network dogechain ${graduationManager.address} ${factory.address} ${router.address} ${DC_TOKEN} ${deployer.address} ${ethers.utils.parseEther("6900")}`);
}
